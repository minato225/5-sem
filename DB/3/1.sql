...
SELECT TO_CHAR(BIRTHDATE, 'DD-MONTH-YEAR') BIRTH_DATE FROM EMP;
--7: тоже но год числом
SELECT TO_CHAR(BIRTHDATE, 'DD-MONTH-YYYY') BIRTH_DATE FROM EMP;

--8: Выдать информацию о должностях, изменив названия должности “CLERK” и “DRIVER” на “WORKER”.
SELECT DECODE(JOBNAME, 'DRIVER', 'WORKER', 'CLERK', 'WORKER', JOBNAME) NEWJOB FROM JOB;
...
SELECT e.EMPNAME, j.JOBNAME, s.SALVALUE, j.MINSALARY FROM EMP e
...
SELECT e.empname, s.salvalue, j.minsalary, j.jobname FROM salary s
...
SELECT e.empname, j.minsalary, j.jobname, d.deptname FROM EMP e
NATURAL JOIN career c
NATURAL JOIN job j
NATURAL JOIN dept d
...
SELECT e.empname, c.startdate, c.enddate FROM emp e 
