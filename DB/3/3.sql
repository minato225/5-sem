...
SELECT (A.EMPNAME || ' manager is ' || B.EMPNAME) EMP_MANAGER FROM EMP A 
...
SELECT (EMPNAME || ' works for ' || PRIOR EMPNAME) EMP_MANAGER FROM EMP
START WITH MANAGER_ID IS NULL
CONNECT BY PRIOR EMPNO = MANAGER_ID; 

-- 3) Требуется показать иерархию от CLARK до JOHN KLINTON.
SELECT LTRIM(SYS_CONNECT_BY_PATH(EMPNAME, ' -> '), ' -> ') EMP_TREE FROM EMP
...
SELECT (LTRIM(SYS_CONNECT_BY_PATH(EMPNAME, ' -> '), ' -> ')) TREE FROM EMP
START WITH MANAGER_ID IS NULL
CONNECT BY PRIOR EMPNO = MANAGER_ID;

-- 5) Требуется показать уровень иерархии каждого сотрудника.
SELECT rpad('*', LEVEL, '*') || EMPNAME TREE FROM EMP
START WITH MANAGER_ID IS NULL
CONNECT BY PRIOR EMPNO = MANAGER_ID
ORDER BY LEVEL;
