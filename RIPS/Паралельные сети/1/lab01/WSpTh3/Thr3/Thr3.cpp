...
unsigned int __stdcall  PaintRound(void* hWnd);

void SuspendEllipse(HMENU hMenu, bool* bSuspend);
void SuspendRectangle(HMENU hMenu, bool* bSuspend);
void SuspendRound(HMENU hMenu, bool* bSuspend);

void TerminateEllipse(HWND hWnd, HMENU hMenu, BOOL* fTerminateE);
void TerminateRound(HWND hWnd, HMENU hMenu, BOOL* fTerminateE);
void TerminateRectangle(HWND hWnd, HMENU hMenu, BOOL* fTerminateE);
...
LONG WndProc_OnCommand(HWND hWnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDM_SUSC:
		SuspendRound(GetMenu(hWnd), &bSuspendC);
		return 0L;
	...
	case IDM_TERMC:
		TerminateRound(hWnd, GetMenu(hWnd), &fTerminateC);
		return 0L;
...
LONG WndProc_OnCreate(HWND hWnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	bSuspendE = false;
	//Thread PaintRound starting not suspended!
	bSuspendC = false;
...
void WaitForAllThreads() {
	...
	for (int i = 0; i < 3; i++) {
		switch (dwRet = WaitForSingleObject(hThreadE[i],  INFINITE))
		{
...
LONG WndProc_OnDestroy(HWND hWnd)
{
	...
	if (!fTerminateR && !CloseHandle(hThreadE[1]))
	{
		MessageBox(NULL, "CloseHandle  failed",
			"PaintRectangule Thread", MB_OK | MB_ICONEXCLAMATION);
	};
	if (!fTerminateC && !CloseHandle(hThreadE[2]))
	{
		MessageBox(NULL, "CloseHandle  failed",
			"PaintRound Thread", MB_OK | MB_ICONEXCLAMATION);
	};
...
unsigned int __stdcall  PaintRound(void* hWnd) {
	...
	srand((unsigned int)hWnd + 100);
	while (!fTerminateC) {	// Is not it to be continued?

		EnterCriticalSection(&critical_section);

		hDC = GetDC((HWND)hWnd);
		nRed = 666 % 255; nGreen = 666 % 255; nBlue = 666 % 255;

		GetWindowRect((HWND)hWnd, &rect);
		xLeft = rand() % (rect.left + 1)+250;
		//xRight = rand() % (rect.right + 1);
		xRight = xLeft + 100;
		yTop = rand() % (rect.top + 1)+150;
		//yBottom = rand() % (rect.bottom + 1);
		yBottom = yTop + 100;
...
void TerminateRound(HWND hWnd, HMENU hMenu, BOOL* fTerminateC)
{
	if (!*fTerminateC)
	{
		//Check
